...
describe('PointsService', () => {
    ...
    describe('getHistory', () => {
        ...
        it('应该正确处理分页边界情况', async () => {
            ...
            // 测试第一页
            await service.getHistory(mockTeamId, { page: 1, limit: 20 });
        ...
        it('应该正确处理businessId和businessType为null的交易记录', async () => {
            const mockTransactionsWithNull = [
                {
                    id: 'tx-3',
                    teamId: mockTeamId,
                    userId: 'user-1',
            ...
            const mockQueryBuilder = {
                ...
                getManyAndCount: jest
                    .fn()
                    .mockResolvedValue([mockTransactionsWithNull, 1]),
            };
            ...
            expect(result.transactions[0].businessId).toBeUndefined();
            expect(result.transactions[0].businessType).toBeUndefined();
        });
    });
    ...
    describe('consume', () => {
        ...
        it('应该正确处理没有businessId和businessType的消耗请求', async () => {
            const requestWithoutBusiness: ConsumePointsRequest = {
                teamId: mockTeamId,
                userId: mockUserId,
                amount: 5,
                description: '简单的积分消耗',
            ...
            const result = await service.consume(requestWithoutBusiness);

            expect(result.transaction.businessId).toBeUndefined();
            expect(result.transaction.businessType).toBeUndefined();
        });
    });
    ...
    describe('recharge', () => {
        ...
        it('应该正确处理没有businessId的充值请求', async () => {
            const requestWithoutBusiness: RechargePointsRequest = {
                teamId: mockTeamId,
                userId: mockUserId,
                amount: 50,
                description: '简单的积分充值',
            ...
            const result = await service.recharge(requestWithoutBusiness);

            expect(result.transaction.businessId).toBeUndefined();
            expect(result.transaction.businessType).toBe('manual');
        });
    });
