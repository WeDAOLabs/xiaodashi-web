...
export class UserLoginLog {
  ...
  @Column({ type: 'uuid', nullable: true, comment: '用户ID，关联到用户表，可为空（登录失败时）' })
  @Index()
  userId?: string;
  ...
  @Column({ type: 'varchar', length: 100, nullable: true, comment: '登录地理位置' })
  location?: string;
  ...
  @Column({ type: 'varchar', length: 100, nullable: true, comment: '登录失败原因' })
