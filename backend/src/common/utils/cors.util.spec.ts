...
describe('CORS Utils', () => {
    describe('isOriginAllowed', () => {
        ...
        it('应该支持多个允许的域名', () => {
            const allowed = ['https://app.sxx0.com', 'https://admin.sxx0.com', '*.test.com'];
            expect(isOriginAllowed('https://app.sxx0.com', allowed)).toBe(true);
            expect(isOriginAllowed('https://admin.sxx0.com', allowed)).toBe(true);
    ...
    describe('parseCorsOrigins', () => {
        ...
        it('应该支持通配符和正则表达式', () => {
            const result = parseCorsOrigins('https://*.sxx0.com,/^https:\\/\\/.*\\.test\\.com$/');
            expect(result).toEqual(['https://*.sxx0.com', '/^https:\\/\\/.*\\.test\\.com$/']);
        });
    });
});
