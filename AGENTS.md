...
Monorepo managed by npm workspaces in the repository root:
...
- `shared/`: TypeScript contracts inside `types/` compiled to `dist/` via `npm run build:shared`; import from this package instead of duplicating types.

## Build, Test, and Development Commands
Execute scripts from the repository root:
- `npm run dev:frontend` — start the marketing site on http://localhost:3000.
- `npm run dev:frontend-app` — launch the console UI, defaulting to http://localhost:3001.
- `npm run dev:backend` — run the NestJS API with file watching.
- `npm run build:all` — compile shared contracts and production bundles for both Next.js apps.
- `npm run lint` — run ESLint across the two frontend workspaces; fix all warnings before committing.
- `npm run test --workspace=backend` — execute the backend Jest suite; add `:cov` when validating coverage locally.
...
## Testing Guidelines
Backend unit specs follow the `*.spec.ts` pattern colocated with source files (see `backend/src/health`). Run `npm run test --workspace=backend` before any push, and prefer `npm run test:cov --workspace=backend` when refining coverage. Frontend automation is not yet scaffolded; place future component specs near their implementation and document manual QA steps in pull requests until tooling lands.
...
Copy `frontend/env.example` to `.env.local` before launching the Next.js apps. Regenerate the backend OpenAPI schema with `npm run openapi:gen --workspace=backend` whenever DTOs change. Ignore generated directories like `.next/`, `dist/`, and `coverage/`.
